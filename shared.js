...
function syncLocalUserState() {
  myName      = localStorage.getItem('mgm_username') || localStorage.getItem('mgm_name') || 'Anonymous';
  ...
  myPeakElo   = parseInt(localStorage.getItem('mgm_peakElo') || localStorage.getItem('mgm_elo') || '400');
  myWinStreak = parseInt(localStorage.getItem('mgm_winStreak') || '0');
  return { loggedIn: !!myUid && !!myUsername };
}

function applyChatAuthState() {
  const state = syncLocalUserState();
  const chatInputRow = document.getElementById('chatInputRow');
  const chatSignin   = document.getElementById('chatSigninPrompt');
  const chatInput    = document.getElementById('chatMsgIn');
  if (chatInputRow) chatInputRow.style.display = state.loggedIn ? 'flex' : 'none';
  if (chatSignin)   chatSignin.style.display   = state.loggedIn ? 'none' : 'block';
  if (chatInput) {
    chatInput.disabled = !state.loggedIn;
    chatInput.placeholder = state.loggedIn ? 'Say something...' : 'Sign in to chat';
  }
}
...
function currentUserPayload() {
  ...
  if (window.mgmIdToken) payload.idToken = window.mgmIdToken;
  return payload;
}

function sendCurrentUserToServer() {
  if (!ws || ws.readyState !== WebSocket.OPEN) return false;
  wsSend({ type:'set_user', ...currentUserPayload() });
  return true;
}

window.mgmSetAuthState = function(payload) {
  payload = payload || {};
  if (payload.uid)      localStorage.setItem('mgm_uid', payload.uid);
  if (payload.name)     localStorage.setItem('mgm_name', payload.name);
  if (payload.username) localStorage.setItem('mgm_username', payload.username);
  if (payload.photoURL) localStorage.setItem('mgm_photo', payload.photoURL);
  if (payload.elo !== undefined)       localStorage.setItem('mgm_elo', payload.elo);
  if (payload.wins !== undefined)      localStorage.setItem('mgm_wins', payload.wins);
  if (payload.losses !== undefined)    localStorage.setItem('mgm_losses', payload.losses);
  if (payload.peakElo !== undefined)   localStorage.setItem('mgm_peakElo', payload.peakElo);
  if (payload.winStreak !== undefined) localStorage.setItem('mgm_winStreak', payload.winStreak);
  if (payload.idToken) window.mgmIdToken = payload.idToken;
  syncLocalUserState();
  applyChatAuthState();
  ...
  switch(msg.type) {
    ...
    case 'match_result':
      ...
      if (msg.newElo !== undefined) {
        ...
        let hist = [];
        try { hist = JSON.parse(localStorage.getItem('mgm_elo_history') || '[]') || []; } catch(e) { hist = []; }
        hist.push(myElo);
        localStorage.setItem('mgm_elo_history', JSON.stringify(hist.slice(-30)));
      }
      if (msg.peakElo !== undefined) { myPeakElo = msg.peakElo; localStorage.setItem('mgm_peakElo', myPeakElo); }
      if (msg.winStreak !== undefined) { myWinStreak = msg.winStreak; localStorage.setItem('mgm_winStreak', myWinStreak); }
      if (msg.dailyChallengeWins !== undefined) localStorage.setItem('mgm_dailyChallengeWins', msg.dailyChallengeWins);
...
function injectSharedUI() {
  ...
  const inboxLink = nav.querySelector('a[href="messages.html"]');
  if (inboxLink) inboxLink.textContent = 'INBOX';
...
function injectDevStatusPanel() {
  ...
  panel.style.cssText = 'position:fixed;left:10px;bottom:10px;z-index:9998;background:rgba(0,0,0,0.72);border:1px solid rgba(74,158,255,0.28);border-radius:8px;padding:8px 10px;font-family:JetBrains Mono,monospace;font-size:9px;line-height:1.6;color:#9fb7d8;pointer-events:none;backdrop-filter:blur(8px);';
  ...
  setInterval(() => {
    ...
    const arenaState = window.arenaState || window.state || '';
    panel.innerHTML = `DEV&nbsp; ${route}<br>WS: ${wsState}<br>AUTH: ${myUid ? myUsername || 'signed-in' : 'guest'}<br>STATE: ${arenaState || '-'}`;
  }, 700);
}
